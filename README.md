...
3. Create a `.env` file in the project root with the following database configuration values:
   - `DB_HOST`
   - `DB_PORT`
   - `DB_NAME`
   - `DB_USER`
   - `DB_PASSWORD`
   - `DB_SSL` (optional, set to `true` or `false`)
4. Use `npm run import:customers` to import `data/customers.csv` into the configured PostgreSQL database.
